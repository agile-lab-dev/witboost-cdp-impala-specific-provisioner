...
class ImpalaAccessControlGatewayTest extends AnyFunSuite with MockFactory {
      ...
      .returns(
        Right(
          RangerRole(
            id = 1,
            isEnabled = true,
            name = "domain_dp_name_0_cmp_name_development_read",
            description = "",
            groups = Seq.empty,
            users = Seq(
              RoleMember("srvRole", isAdmin = true),
  ...
  // TEST OP VIEW
  test("provision access control for view") {
    val datalake = new Datalake()
    datalake.setEnvironmentCrn("cdpEnvCrn")
    datalake.setDatalakeName("dlName")
    ...
    (rangerRoleGateway.upsertRole _)
      .when("domain_dp-name_0_cmp-name-view", UserRoleType, *, *, *, *, *)
      .returns(
        Right(
          RangerRole(
            id = 1,
            isEnabled = true,
            name = "domain_dp_name_0_cmp_name_view_read",
            description = "",
            groups = Seq.empty,
            users = Seq(
              RoleMember("srvRole", isAdmin = true)
    ...
    val accessControlInfo = AccessControlInfo(
      dataProductOwner = "dataProductOwner",
      devGroup = "devGroup",
      componentId = "urn:dmb:cmp:domain:dp-name:0:cmp-name-view"
    )
      ...
      .returns(
        Right(
          RangerRole(
            id = 1,
            isEnabled = true,
            name = "domain_dp_name_0_cmp_name_view_development_read",
            description = "",
            groups = Seq.empty,
            users = Seq(
              RoleMember("srvRole", isAdmin = true),
